...
public class PlagiarismChecker {
    ...
    // This method was my attempt to return all of the longest shared substrings
    public static int attemptlongestSharedSubstring(String doc1, String doc2) {
        // 2D array representing possible paths for substrings
            ...
            for (int j = 0; j < doc2.length(); j++) {
                ...
                if (doc1.charAt(i) == doc2.charAt(j)) {
                    ...
                    if (i > 0 && j > 0) {
                        ...
                        for (String sub : path[i - 1][j - 1].keySet()) {
                            path[i][j].put(sub + newChar, sub.length() + 1);
                        }
                        if (path[i][j].isEmpty()) {path[i][j].put(newChar + "", 1);}
                    }
